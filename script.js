(function(){
  function init(){
    ...
    if(!canvas || !spinBtn || !resultEl || !logEl || !legendEl){
      console.warn('FortuneFi init: missing DOM nodes');
      return;
    }

    const ctx = canvas.getContext('2d');
    if(!ctx){
      resultEl.textContent = 'Canvas wordt niet ondersteund door deze browser.';
      return;
    }
    ...
    logo.onerror = drawWheel; // teken ook zonder logo
    ...
    function spin() {
      ...
      const delta = (function shortestRotation(from, to){
        let d = to - from;
      ...
      function frame(now) {
        ...
        if (t < 1) {
          ...
          prependLog(`[${new Date().toLocaleTimeString()}] WIN → ${win.label}`);
          spinning = false;
    ...
    // Initial draw (in case logo is cached and already complete)
