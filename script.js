...
function drawWheel() {
  ...
  ctx.beginPath(); ctx.arc(0, 0, radius * 0.18 + 6, 0, Math.PI * 2);
  ...
  if (logo && logo.complete) {
    ...
    ctx.globalAlpha = 1; ctx.drawImage(logo, -s/2, -s/2, s, s);
    ctx.restore();
  }
...
function wrapText(ctx, text, x, y, maxWidth, lineHeight) {
  ...
  const totalH = lines.length * lineHeight; let yy = y - totalH/2 + lineHeight*0.85;
...
function weightedChoice(items, weightKey = 'weight') {
  ...
  for (let i = 0; i < items.length; i++) { r -= (items[i][weightKey] || 0); if (r <= 0) return i; }
  return items.length - 1;
}

// ✅ Uitlijnen op BOVEN bij de neerwaartse pijl (-90°)
...
function shortestRotation(from, to) { let d = to - from; while (d <= 0) d += Math.PI * 2; return d; }
